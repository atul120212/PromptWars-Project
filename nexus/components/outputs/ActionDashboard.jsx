...
export default function ActionDashboard({ result, isLoading, onFeedback }) {
        ...
        return (
            <div className={styles.loadingState}>
                ...
                <div className={styles.loadingSteps}>
                    {['🎯 Extracting Intent', '🧠 Understanding Context', '✅ Verifying Actions', '⚡ Generating Plan'].map((step, i) => (
        ...
        return (
            <div className={styles.emptyState}>
                ...
                <div className={styles.emptyDesc}>
                    Add a voice message, upload an image, or type any messy real-world text on the left.
                    IntentBridge AI will instantly convert it into structured, verified actions.
                </div>
                <div className={styles.emptyFeatures}>
                    {['Medical Triage', 'Emergency Dispatch', 'Traffic Routing', 'Crisis Intel', 'Environmental Alerts'].map(f => (
    ...
    const urgencyCounts = actions.reduce((acc, a) => {
        acc[a.urgency] = (acc[a.urgency] || 0) + 1;
        return acc;
    }, {});

    const confidenceColor = data.confidence >= 80 ? 'var(--accent-green)'
        : data.confidence >= 50 ? 'var(--urgency-medium)'
            : 'var(--urgency-critical)';

    return (
        <div className={styles.dashboard}>
            {/* Summary Header */}
            <div className={`${styles.summaryHeader} domain-bg-${data.domain || 'GENERAL'}`}>
                <div className={styles.summaryLeft}>
                    <div className={styles.intentText}>{data.intent}</div>
                    <div className={styles.summaryMeta}>
                        <span className={`badge badge-${(data.urgency || 'LOW').toLowerCase()}`}>
                            {data.urgency}
                        </span>
            ...
            {data.verification?.requires_human_confirmation && (
                <div className={styles.confirmAlert}>
                    ⚠️ <strong>Human Confirmation Required</strong> — Review all actions before executing.
            ...
            {actions.length > 1 && (
                <div className={styles.filters}>
                    {['ALL', 'CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].map(f => (
                        ...
                        >
                            {f} {f !== 'ALL' && urgencyCounts[f] ? `(${urgencyCounts[f]})` : ''}
                        </button>
                    ))}
                </div>
            )}
            ...
            <div className={styles.actionsList}>
                ...
                {filtered.length === 0 ? (
                    <div className={styles.noActions}>No actions with this urgency level</div>
                ) : (
            ...
            {data.safetyChecks?.length > 0 && (
                <div className={styles.safetySection}>
                    ...
                    >
                        🛡️ Safety Checks ({data.safetyChecks.filter(s => s.passed).length}/{data.safetyChecks.length} passed)
                        <span>{showVerification ? '▲' : '▼'}</span>
                    </button>
                    {showVerification && (
                        <div className={styles.safetyList}>
                            {data.safetyChecks.map((check, i) => (
                                <div key={i} className={`${styles.safetyItem} ${check.passed ? styles.safetyPassed : styles.safetyFailed}`}>
                                    {check.passed ? '✅' : '❌'} <strong>{check.check}</strong>
                                    {check.note && <span className={styles.safetyNote}> — {check.note}</span>}
                                </div>
                            ))}
                        </div>
                    )}
