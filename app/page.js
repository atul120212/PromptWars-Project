...
const DEMO_STREAM = [
  { delay: 0, text: '📥 Input: "dad sweating not responding highway 5"', type: 'input' },
  { delay: 1200, text: '🧠 Detecting intent...', type: 'process' },
  { delay: 2200, text: '⚡ Intent: Medical Emergency • Severity: CRITICAL', type: 'output' },
  { delay: 3000, text: '✅ Action: Call 911 immediately', type: 'action' },
  { delay: 3600, text: '✅ Action: Locate nearest cardiac unit', type: 'action' },
  { delay: 4200, text: '✅ Action: Alert family members', type: 'action' },
  { delay: 5000, text: '📥 Input: [PDF — Medical History Upload]', type: 'input' },
  { delay: 6200, text: '🧠 Extracting medical entities...', type: 'process' },
  { delay: 7200, text: '⚡ Found: Diabetes, Hypertension, Metformin 500mg', type: 'output' },
  { delay: 8000, text: '✅ Action: Drug interaction check — flagged', type: 'action' },
...
export default function HomePage() {
  ...
  return (
    <div className={styles.page}>
      ...
      {/* Nav */}
      <nav className={styles.nav} style={{ background: scrollY > 20 ? 'rgba(8,11,20,0.9)' : 'transparent' }}>
      ...
      <section className={styles.hero}>
        <div className={styles.heroContent}>
          ...
          <h1 className={styles.heroTitle}>
            Turning Human{' '}
            <span className={styles.heroHighlight}>Chaos</span>
            {' '}into{' '}
          ...
          <p className={styles.heroSub}>
            Throw any messy real&#8209;world input at IntentBridge AI — voice, photos, medical history,
            traffic data, breaking news — and instantly get structured, verified, actionable outputs
            that connect humans to the systems that matter most.
          </p>
          ...
          <div className={styles.heroCTA}>
            ...
            <a href="#domains" className="btn btn-secondary" style={{ fontSize: 16, padding: '14px 28px' }}>
              See Use Cases
            </a>
          </div>
        ...
        <div className={styles.heroDemo}>
          ...
          <div className={styles.demoWindow}>
            <div className={styles.demoWindowBar}>
              ...
              <span className={styles.demoWindowTitle}>IntentBridge AI — Live Processing</span>
            </div>
            <div className={styles.demoBody} ref={demoRef}>
              {demoItems.map((item, i) => (
                <div key={i} className={`${styles.demoLine} ${styles['demoLine_' + item.type]}`}
      ...
      <section id="domains" className={styles.domains}>
        <div className={styles.sectionHeader}>
          ...
          <p className={styles.sectionSub}>
            One AI, infinite real-world scenarios. IntentBridge intelligently routes your input
            to the right domain and generates precise, verified actions.
          </p>
        </div>

        <div className={styles.domainGrid}>
          {DOMAINS.map((d, i) => (
            <Link href={`/dashboard?domain=${d.id}`} key={d.id}
              className={`${styles.domainCard} glass-card`}
              style={{ animationDelay: `${i * 0.1}s` }}>
              <div className={styles.domainIcon} style={{ background: `${d.color}18`, borderColor: `${d.color}30` }}>
                {d.icon}
              </div>
              <h3 className={styles.domainTitle} style={{ color: d.color }}>{d.title}</h3>
              <p className={styles.domainDesc}>{d.desc}</p>
              <div className={styles.domainExample}>
                <span className={styles.domainExampleLabel}>Example input:</span>
                <span className={styles.domainExampleText}>{d.example}</span>
              </div>
              <div className={styles.domainArrow} style={{ color: d.color }}>Try this domain →</div>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className={styles.howItWorks}>
        <div className={styles.sectionHeader}>
          ...
          <p className={styles.sectionSub}>Four intelligent layers, zero manual structuring</p>
        </div>

        <div className={styles.pipeline}>
          {[
            { n: '01', icon: '🎯', title: 'Ingest Any Input', desc: 'Voice, images, text, PDFs, data streams — any format accepted' },
            { n: '02', icon: '🧠', title: 'Gemini Understands', desc: 'Extracts intent, urgency, entities and context with high accuracy' },
            { n: '03', icon: '✅', title: 'Verify & Validate', desc: 'Cross-checks facts, removes hallucinations, assigns confidence scores' },
            { n: '04', icon: '⚡', title: 'Actionable Output', desc: 'Structured JSON actions with urgency levels, safety notes, and next steps' },
          ].map((step, i) => (
      ...
      <section className={styles.ctaSection}>
        ...
        <p className={styles.ctaSub}>Start processing real-world chaos into life-saving actions</p>
        <Link href="/dashboard" className="btn btn-primary" style={{ fontSize: 17, padding: '16px 40px' }}>
