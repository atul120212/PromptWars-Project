...
function DashboardContent() {
  ...
  const saveSession = (input, res) => {
    ...
    const newSession = {
      ...
      inputType: inputTab,
      inputPreview:
        typeof input === "string" ? input.slice(0, 120) : "Image/File",
    };
  ...
  const runAnalysis = useCallback(
    async (type, content, imageBase64 = null, imageMimeType = null) => {
  ...
  const handleTextSubmit = useCallback((text) => runAnalysis("text", text), [runAnalysis]);
  const handleVoiceTranscript = useCallback((transcript) => runAnalysis("voice", transcript), [runAnalysis]);
  ...
  const handleFeedback = useCallback((actionId, type) => {
        ...
        <div className={styles.rightPanel}>
          <div className={styles.panelHeader}>
            ...
            <span className={styles.panelSub}>
              Verified • Actionable • Life-critical
            </span>
          </div>
